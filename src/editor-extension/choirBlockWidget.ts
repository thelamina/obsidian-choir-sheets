...
export class ChoirBlockGroupWidget extends WidgetType {
	constructor(
	...
	eq(other: ChoirBlockGroupWidget): boolean {
		if (this.blocks.length !== other.blocks.length) return false;
		for (let i = 0; i < this.blocks.length; i++) {
			if (this.blocks[i].source !== other.blocks[i].source) return false;
			if (this.blocks[i].part !== other.blocks[i].part) return false;
		}
		return true;
	}
	...
	toDOM(view: EditorView): HTMLElement {
		...
		wrapper.className = 'choir-block-wrapper';
		this.applyColors(wrapper);

		const tabBar = document.createElement('div');
		tabBar.className = 'choir-tab-bar';
		wrapper.appendChild(tabBar);

		const tabContents = document.createElement('div');
		tabContents.className = 'choir-tab-contents';
		wrapper.appendChild(tabContents);

		const allContents = document.createElement('div');
		allContents.className = 'choir-all-contents';
		...
		this.blocks.forEach((block) => {
			...
			tabBtn.addEventListener('click', (e) => {
				...
				if (activeTabs.length === 0) {
					...
					contentContainer.querySelectorAll('.choir-block').forEach(c => c.classList.add('hidden'));
					const activeBlock = contentContainer.querySelector(`.choir-block[data-part="${activeTab.dataset.part}"]`);
